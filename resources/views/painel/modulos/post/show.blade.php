...
@section('conteudo')
    <div class="title-pg">
        <h1 class="title-pg">Visualizar Posts: {{$data->name}}</h1>
    </div>
        ...
        <div class="box-body">
            <div class="row">
                <div class="col-md-8">
                    ...
                    <h4><strong>User ID: </strong>{{$data->user_id}}</h4>
                    <h4><strong>Descrição: </strong>{{$data->description}}</h4>

                </div>
                <div class="col-md-4">
                    @if(isset($data->image))
                        <img src="{{URL::asset('assets/uploads/posts/'.$data->image)}}" alt="$data->image" class="img-responsive img-rounded img-bordered">
                    @endif
                </div>
            </div>
        </div>
